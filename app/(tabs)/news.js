...
function ActionButton({ icon, active, activeColor, label, onPress }) {
  return (
    <Pressable onPress={onPress} hitSlop={8} style={{ alignItems:"center", gap:4 }}>
      ...
      <Text style={{ fontSize:9, color:"white", fontWeight: active?"600":"400", textShadowColor:"rgba(0,0,0,0.8)", textShadowRadius:3 }}>
        {label}
      </Text>
    </Pressable>
  );
...
function ReelCard({ article, cardHeight, saved, onSave, reaction, onReact }) {
  const imgUrl    = CATEGORY_IMAGES[article.category] || DEFAULT_IMAGE;
  const catColor  = CATEGORY_COLORS[article.category] || "#1a7a69";
  const halfHeight = cardHeight * 0.45;
  const likeColor = "#1d9e87";
  const saveColor = "#1d9e87";

  return (
    <View style={{ width:"100%", height:cardHeight, backgroundColor:C.cream }}>
      ...
      <View style={{ height:halfHeight, position:"relative" }}>
        ...
        {/* Gradiente inferior (para fundir con texto) */}
        <LinearGradient
        ...
        {/* Botones laterales */}
        <View style={{ position:"absolute", right:12, top:"50%", transform:[{translateY:-125}], gap:10, alignItems:"center" }}>
          <ActionButton icon={reaction==="like"?"heart":"heart-outline"}
            active={reaction==="like"} activeColor={likeColor} label="Me gusta"
            onPress={() => onReact(article.id, "like")}/>
          <ActionButton icon={reaction==="dislike"?"thumbs-down":"thumbs-down-outline"}
            active={reaction==="dislike"} activeColor="#d4522a" label="No me gusta"
            onPress={() => onReact(article.id, "dislike")}/>
          <ActionButton icon={saved?"bookmark":"bookmark-outline"}
            active={saved} activeColor={saveColor} label={saved?"Guardado":"Guardar"}
            onPress={() => onSave(article.id)}/>
          <ActionButton icon="open-outline"
            active={false} activeColor="white" label="Leer"
            onPress={() => article.source_url && Linking.openURL(article.source_url)}/>
          <ActionButton icon="share-social-outline"
            active={false} activeColor="white" label="Compartir"
            onPress={() => handleShare(article)}/>
        </View>
      </View>

      {/* Mitad inferior — texto */}
      <View style={{ flex:1, paddingHorizontal:18, paddingTop:4, paddingBottom:16 }}>
...
export default function News() {
  ...
  const [saved, setSaved]         = useState({});         // {articleId: true}
  const [reactions, setReactions] = useState({});          // {articleId: "like"|"dislike"}
  const [profileId, setProfileId] = useState(null);
  const insets = useSafeAreaInsets();
  const { height: windowHeight } = useWindowDimensions();

  // Altura de cada reel = pantalla - tabbar - top safe area
  ...
  async function loadAll() {
    ...
    try {
      ...
      // Guardados previos (botiquín)
      const { data:savedRows } = await supabase
        .from("user_saved_content").select("content_id")
        .eq("user_id", profile.id).eq("content_type", "news");
      const savedMap = {};
      ...
      const { data:all } = await supabase
        .from("news").select("*").order("published_at", { ascending:false }).limit(50);

      // Filtrar las que no me gustaron
      const dislikedIds = (prevReactions || []).filter(r => r.reaction === "dislike").map(r => r.article_id);
      const filtered = (all || []).filter(a => !dislikedIds.includes(a.id));
  ...
  async function handleReact(articleId, type) {
    if(!profileId) return;
    ...
    try {
      ...
      // Si es "no me gusta", quitar de la lista tras 0.8s
    ...
    try {
      if(wasSaved) {
        await supabase.from("user_saved_content").delete()
          .eq("user_id", profileId).eq("content_id", articleId).eq("content_type", "news");
      } else {
        await supabase.from("user_saved_content").insert({
          user_id: profileId, content_id: articleId, content_type: "news",
        });
      }
    } catch(err) {
      setSaved(s => ({ ...s, [articleId]: wasSaved }));
