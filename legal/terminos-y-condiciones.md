# Términos y Condiciones de Uso

**Última actualización:** 16 de mayo de 2026
**Versión:** 1.0

---

## 1. Aceptación

Estos Términos y Condiciones de Uso (en adelante, "**Términos**") regulan la relación entre **Antonio Martínez Sancho**, con NIF 53509031Z y domicilio en Calle Villabona 11 interior, 33520 Nava, Asturias, España (en adelante, "**el Titular**"), y cualquier persona física que se registre y utilice la aplicación móvil **SmartPills** (en adelante, "**la Aplicación**" o "**el Servicio**", y la persona registrada, "**el Usuario**").

Al crear una cuenta, marcar la casilla de aceptación correspondiente o utilizar la Aplicación, el Usuario declara:

a) Haber leído, comprendido y aceptado estos Términos en su totalidad;
b) Haber leído la **Política de Privacidad** y el **Aviso Legal**, que forman parte integrante de estos Términos;
c) Cumplir los requisitos de elegibilidad descritos en la cláusula 3.

Si el Usuario no acepta estos Términos, deberá abstenerse de registrarse y utilizar la Aplicación.

---

## 2. Descripción del Servicio

SmartPills es una aplicación móvil de información, formación y actualización profesional dirigida a profesionales sanitarios. Sus principales funcionalidades son:

- **Pills:** feed de artículos científicos y noticias sanitarias resumidos.
- **Guías clínicas:** enlaces a guías oficiales de sociedades científicas con resumen de cambios.
- **Resúmenes e infografías:** material visual elaborado a partir de fuentes científicas.
- **Tests y retos:** preguntas de evaluación de conocimiento.
- **Gamificación:** sistema de puntos de experiencia (XP), ligas, ranking y rachas.
- **Botiquín personal:** contenido guardado por el Usuario.

El Titular podrá añadir, modificar o eliminar funcionalidades en cualquier momento sin previo aviso.

---

## 3. Elegibilidad y registro

### 3.1. Requisitos del Usuario

Para registrarse y utilizar SmartPills, el Usuario debe:

- Ser **mayor de 18 años**.
- Ser **profesional sanitario** colegiado en activo, profesional sanitario en formación, o estudiante universitario de Ciencias de la Salud, o cualquier persona con interés profesional legítimo en contenido sanitario.
- Tener capacidad legal para vincularse contractualmente conforme a la legislación española.
- Proporcionar información veraz, completa y actualizada al registrarse.

El Titular se reserva el derecho a solicitar acreditación profesional al Usuario, así como a denegar el acceso o cancelar la cuenta de quien no cumpla estos requisitos.

### 3.2. Cuenta del Usuario

El registro requiere correo electrónico, contraseña y la cumplimentación de un perfil profesional. El Usuario:

- Es responsable de mantener la confidencialidad de sus credenciales.
- No podrá ceder, vender ni compartir su cuenta con terceros.
- Notificará al Titular inmediatamente cualquier acceso no autorizado.
- Es responsable de toda actividad realizada desde su cuenta.

El Titular podrá suspender cuentas con sospechas de uso fraudulento o suplantación.

---

## 4. Uso aceptable

### 4.1. Compromisos del Usuario

El Usuario se compromete a utilizar SmartPills de buena fe y conforme a la ley. En particular, se obliga a:

- Utilizar la Aplicación con fines profesionales, formativos o informativos.
- No publicar, transmitir o difundir contenido ilícito, ofensivo, discriminatorio, difamatorio o que vulnere derechos de terceros.
- No utilizar la Aplicación para realizar actividades comerciales no autorizadas, publicidad, spam o captación de pacientes.
- Respetar la confidencialidad de los datos de pacientes; **el Usuario nunca debe introducir, compartir ni almacenar en SmartPills datos identificativos de pacientes reales**.
- Respetar las normas de propiedad intelectual del contenido y de terceros.

### 4.2. Conductas prohibidas

Queda expresamente prohibido:

- Acceder a la Aplicación mediante medios automatizados, scrapers o bots no autorizados.
- Realizar ingeniería inversa, descompilar o intentar acceder al código fuente de la Aplicación.
- Eludir o intentar eludir medidas técnicas de seguridad o de control de acceso.
- Suplantar la identidad de otra persona o falsear datos profesionales.
- Manipular el sistema de gamificación (XP, ligas, ranking) mediante trampas o cuentas múltiples.
- Reproducir, redistribuir o explotar comercialmente el contenido de SmartPills sin autorización expresa.
- Utilizar SmartPills para tomar decisiones clínicas individuales sin el debido contraste con fuentes oficiales (véase cláusula 6).

El incumplimiento podrá motivar la suspensión inmediata de la cuenta y, en su caso, las acciones legales correspondientes.

---

## 5. Contenido y propiedad intelectual

### 5.1. Contenido del Servicio

Todo el contenido de SmartPills (textos, imágenes propias, diseño, código, marca, logotipos) es propiedad del Titular o de terceros que han autorizado su uso. La Aplicación se entrega bajo una licencia de uso personal, gratuita, no exclusiva, no transferible y revocable.

### 5.2. Contenido de terceros

SmartPills incluye contenido elaborado por terceros (publicaciones científicas, sociedades médicas, organismos oficiales). El Titular respeta los derechos de propiedad intelectual de dichos terceros y se acoge al derecho de cita reconocido en el artículo 32 LPI. La fuente original siempre se identifica y, cuando es posible, se enlaza.

Si algún titular de derechos considera vulnerados sus derechos, podrá contactar en smartpillsapp@gmail.com para la retirada inmediata del contenido afectado.

### 5.3. Contenido generado por el Usuario

Si el Usuario aporta contenido (comentarios, sugerencias, etc.), concede al Titular una licencia mundial, gratuita, no exclusiva, transferible y por todo el plazo de protección legal, para utilizar, reproducir, modificar y comunicar dicho contenido en relación con el Servicio.

El Usuario garantiza que el contenido aportado es original y no infringe derechos de terceros, y mantendrá indemne al Titular frente a cualquier reclamación derivada.

---

## 6. Aviso médico y limitaciones del Servicio

### 6.1. Naturaleza informativa y no clínica

**SmartPills es una herramienta de información, formación y actualización profesional. NO ES UN PRODUCTO SANITARIO** en el sentido del Reglamento (UE) 2017/745 sobre productos sanitarios. La Aplicación:

- **No realiza diagnósticos.**
- **No recomienda tratamientos individualizados.**
- **No calcula dosis ni indica posologías personalizadas.**
- **No sustituye el juicio clínico** del profesional ni la consulta de fuentes oficiales (fichas técnicas AEMPS, guías oficiales en su versión completa, literatura científica primaria).

### 6.2. Responsabilidad del Usuario en su práctica clínica

El Usuario reconoce y acepta que cualquier decisión clínica adoptada en su práctica profesional es de su exclusiva responsabilidad. SmartPills se ofrece **"tal cual"**, sin ninguna garantía sobre la exactitud, exhaustividad, vigencia o adecuación del contenido para un caso clínico concreto.

El Usuario se compromete a **contrastar siempre** la información de SmartPills con la fuente original y con la literatura científica de referencia antes de aplicarla en el ejercicio profesional.

### 6.3. Contenido generado por inteligencia artificial

Parte del contenido de SmartPills se genera o asiste mediante sistemas de inteligencia artificial (resúmenes, infografías, ideas clave). Conforme al Reglamento (UE) 2024/1689 (AI Act):

- Dicho contenido se identifica visiblemente como generado por IA.
- El Usuario reconoce que el contenido generado por IA **puede contener errores, omisiones, sesgos o información desactualizada** ("alucinaciones").
- El Titular implementa procesos de revisión razonables, pero **no garantiza la corrección absoluta** del contenido generado por IA.

El Usuario asume expresamente el deber de contrastar la información con fuentes primarias antes de cualquier uso clínico.

---

## 7. Gamificación

SmartPills incluye un sistema de gamificación basado en puntos de experiencia (XP), ligas, ranking y rachas, cuya finalidad es motivar el aprendizaje y la actualización profesional.

- Los puntos, ligas y rachas **no tienen valor económico** y no son canjeables por dinero, bienes o servicios.
- El Titular puede modificar las reglas de gamificación, recalcular puntuaciones, anular cuentas o suspender el sistema en cualquier momento sin que ello genere derecho a indemnización.
- El uso de trampas, cuentas múltiples u otras técnicas para manipular las puntuaciones es motivo de cancelación de la cuenta.

El Usuario acepta que su nombre de usuario, profesión y, en su caso, ciudad/centro de trabajo, **podrán ser visibles** en rankings y ligas para los demás Usuarios, salvo que se habilite y active expresamente una opción de anonimato.

---

## 8. Modificaciones

El Titular podrá modificar estos Términos para adaptarlos a cambios normativos, técnicos o del propio Servicio. Las modificaciones se comunicarán dentro de la Aplicación con una antelación razonable. El uso continuado del Servicio tras la entrada en vigor de las modificaciones implicará su aceptación.

Si el Usuario no estuviera de acuerdo con la nueva versión, podrá cancelar su cuenta antes de su entrada en vigor.

---

## 9. Suspensión y terminación

### 9.1. Por el Usuario

El Usuario podrá cancelar su cuenta en cualquier momento desde el apartado correspondiente del perfil, lo que implicará la eliminación de sus datos personales conforme a lo dispuesto en la Política de Privacidad.

### 9.2. Por el Titular

El Titular podrá suspender o cancelar la cuenta del Usuario, total o parcialmente, sin necesidad de preaviso, en los siguientes casos:

- Incumplimiento grave o reiterado de estos Términos.
- Sospecha fundada de uso fraudulento o suplantación.
- Cumplimiento de obligaciones legales, requerimientos judiciales o administrativos.
- Cese del Servicio.

La cancelación no generará derecho a indemnización a favor del Usuario, sin perjuicio de los derechos del Usuario respecto de sus datos personales (véase la Política de Privacidad).

---

## 10. Limitación de responsabilidad

En la medida máxima permitida por la legislación aplicable, el Titular no será responsable de:

- Daños indirectos, lucro cesante, pérdida de datos, pérdida de oportunidad o daño reputacional.
- Daños derivados de decisiones clínicas adoptadas por el Usuario sobre la base del contenido de la Aplicación.
- Daños derivados de la indisponibilidad temporal del Servicio, errores técnicos, virus, ataques informáticos o causas de fuerza mayor.
- Daños derivados de la interacción del Usuario con servicios de terceros enlazados desde la Aplicación.

La responsabilidad total del Titular, en cualquier caso, quedará limitada a las cantidades efectivamente abonadas por el Usuario al Titular en los doce (12) meses anteriores al hecho que motive la reclamación, o a 100 euros si el Servicio se ha utilizado de forma gratuita.

Estas limitaciones no excluyen la responsabilidad del Titular por dolo, culpa grave o cualquier otro supuesto en que la limitación esté prohibida por la legislación aplicable.

---

## 11. Indemnización

El Usuario se compromete a mantener indemne al Titular frente a cualquier reclamación, sanción, daño, pérdida, gasto u honorarios de letrados que se deriven del incumplimiento por su parte de estos Términos, de la legislación aplicable o de derechos de terceros.

---

## 12. Datos personales

El tratamiento de los datos personales del Usuario se rige por la **Política de Privacidad**, accesible dentro de la Aplicación, que cumple con el Reglamento (UE) 2016/679 (RGPD) y la Ley Orgánica 3/2018, de Protección de Datos Personales y Garantía de los Derechos Digitales (LOPDGDD).

---

## 13. Cesión

El Usuario no podrá ceder, transmitir o subrogar sus derechos u obligaciones derivados de estos Términos sin consentimiento previo y por escrito del Titular. El Titular podrá ceder la posición contractual a terceros, en particular en el marco de operaciones societarias (fusión, escisión, venta de activos), notificándolo al Usuario.

---

## 14. Nulidad parcial

Si alguna cláusula de estos Términos fuera declarada nula o inaplicable por una autoridad competente, dicha declaración no afectará a la validez del resto. La cláusula nula se sustituirá por la disposición legal o jurisprudencial más próxima a la voluntad de las partes.

---

## 15. Legislación aplicable y jurisdicción

Estos Términos se rigen por la legislación española. Para cualquier controversia derivada de la interpretación o aplicación de estos Términos, las partes, con renuncia expresa a su propio fuero si lo tuvieren, se someten a los **Juzgados y Tribunales de Oviedo (Asturias)**, salvo que la legislación aplicable a consumidores y usuarios establezca un fuero distinto.

El Usuario consumidor podrá acudir a la plataforma europea de resolución de litigios en línea: <https://ec.europa.eu/consumers/odr>.

---

## 16. Contacto

Para cualquier consulta sobre estos Términos:

- **Email:** smartpillsapp@gmail.com
- **Dirección postal:** Calle Villabona 11 interior, 33520 Nava, Asturias, España

---

*El Usuario declara haber leído y comprendido estos Términos antes de aceptarlos. La versión vigente está disponible dentro de la Aplicación, en el apartado "Información legal" del perfil del Usuario.*
